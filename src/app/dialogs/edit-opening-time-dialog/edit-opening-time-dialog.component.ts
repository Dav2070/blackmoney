...
export class EditOpeningTimeDialogComponent {
	...
	onDurchgehendChange(block: Block) {
		if (block.durchgehend) {
			block.pause = false
		}
	}

	onPauseChange(block: Block) {
		if (block.pause) {
	...
	groupDaysByTime(days: Day[]): Block[] {
		...
		while (unassigned.length > 0) {
			...
			const same = unassigned.filter(d =>
	...
	getAvailableDays(index: number): string[] { 
		const selectedInOtherBlocks = this.blocks.filter((_, i) => i !== index).flatMap(b => b.selectedDays); 
		return this.weekdays.filter(day => !selectedInOtherBlocks.includes(day)); 
	}

	sortDays(days: Day[]): Day[] { 
		return [...days].sort((a, b) => this.weekdays.indexOf(a.day) - this.weekdays.indexOf(b.day) ); 
	}
