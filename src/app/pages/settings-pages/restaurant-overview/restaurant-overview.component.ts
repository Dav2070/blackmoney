...
import {
	...
	OpeningDaysGroup,
	SpecialDay
} from "src/app/models/settings-models/restaurant-settings.model"
...
export class RestaurantOverviewComponent {
			...
			tses: [
				{
					...
					status: "aktiv",
					pin: "123456",
					clients: [{ id: 1, name: "Kasse 1", seriennummer: "SN-123456" }]
				}
			]
		},
		{
	...
	addGroup() {
		this.restaurant.openingDaysGroups.push({
			days: [],
			periods: [{ from: "", to: "" }]
		})
	}
	...
	validate(): string[] {
		...
		const dayPeriods: {
			[day: string]: { from: number; to: number; group: number }[]
		} = {}

		for (const [i, group] of this.restaurant.openingDaysGroups.entries()) {
			if (!group.days.length) {
				errors.push(
					`Gruppe ${i + 1}: Bitte mindestens einen Tag auswählen.`
				)
			}
			...
			for (const [j, period] of group.periods.entries()) {
				if (!period.from || !period.to) {
					errors.push(
						`Gruppe ${i + 1}, Zeitraum ${j + 1}: Bitte beide Zeiten angeben.`
					)
				} else if (period.from === period.to) {
					errors.push(
						`Gruppe ${i + 1}, Zeitraum ${j + 1}: Die Startzeit darf nicht gleich der Endzeit sein.`
					)
				}
			}
		...
		for (const day of allDays) {
			...
			for (let i = 0; i < periods.length; i++) {
				for (let j = i + 1; j < periods.length; j++) {
					...
					if (a.group === b.group && a.from === b.from && a.to === b.to)
						continue
	...
	addClient(tse: TSE) {
		const nextId = tse.clients.length
			? Math.max(...tse.clients.map(c => c.id)) + 1
			: 1
		tse.clients.push({
			id: nextId,
			name: "Neuer Client",
	...
	setSpecialDayType(
		day: SpecialDay,
		type: "geschlossen" | "durchgehend" | "pause"
	) {
		day.openType = type
		if (type === "geschlossen") {
			day.periods = []
