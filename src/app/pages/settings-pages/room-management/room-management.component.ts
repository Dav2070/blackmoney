...
export class RoomManagementComponent {
  ...
  addTable() {
    if (!this.selectedRoom || this.newTableNumber == null || this.newTableSeats == null) return;
  ...
  prepareMultiTables() {
    if (!this.selectedRoom || !this.multiTableCount || !this.multiTableStartNumber || !this.multiTableSeats) return;
    this.multiTablesToAdd = [];
    let start = this.multiTableStartNumber;
    for (let i = 0; i < this.multiTableCount; i++) {
      this.multiTablesToAdd.push({ number: start + i, seats: this.multiTableSeats });
    }
  }
  ...
  removeTable(table: Table) {
    if (!this.selectedRoom) return;
    this.selectedRoom.tables = this.selectedRoom.tables.filter(t => t.uuid !== table.uuid);
  ...
  addCombination() {
    if (!this.selectedRoom || this.combinationTables.length < 2 || !this.combinationSeats) return;
    const exists = this.selectedRoom.combinations.some(
      c => c.tableUuids.length === this.combinationTables.length &&
  ...
  removeCombination(combi: TableCombination) {
    if (!this.selectedRoom) return;
    this.selectedRoom.combinations = this.selectedRoom.combinations.filter(c => c !== combi);
  }
